...
const Homepage = (props) => {
  ...
  useEffect(() => {
    ...
    pathParams = launchSuccess
      ? `${pathParams}&launch_success=${launchSuccess}`
      : `${pathParams}`;

    pathParams = landSuccess
      ? `${pathParams}&land_success=${landSuccess}`
      : `${pathParams}`;

    pathParams = selectedLaunchYear
      ? `${pathParams}&launch_year=${selectedLaunchYear}`
      : `${pathParams}`;

    setPathParamUrl(pathParams);

    const fetchData = async () => {
      return axios.get(`${url}${pathParamUrl}`).then((response) => {
  ...
  const handleLandFilter = (value) => {
    landSuccess && landSuccess === value
      ? setLandSuccess()
      : setLandSuccess(value);
  };

  const handleLaunchFilter = (value) => {
    launchSuccess && launchSuccess === value
      ? setLaunchSuccess()
      : setLaunchSuccess(value);
  };

  const handleYearFilter = (value) => {
    selectedLaunchYear && selectedLaunchYear === value
      ? setSelectedLaunchYear()
      : setSelectedLaunchYear(value);
  };
